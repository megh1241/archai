...
def find_measures_arrays(
    ...
    if dataload == "random":
        inputs, targets = get_some_data(
            args, trainloader, num_batches=num_imgs_or_batches, device=device
        )
    elif dataload == "grasp":
        inputs, targets = get_some_data(
            args, trainloader, num_batches=num_imgs_or_batches, device=device
        )
        # TODO: is this even possible for lm datasets??
        # inputs, targets = get_some_data_grasp(args, trainloader, num_classes, samples_per_class=num_imgs_or_batches, device=device)
    else:
